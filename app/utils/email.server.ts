  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f3f4f6;padding:40px 16px;">
    <tr><td align="center">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;">
        ...
        <!-- Header -->
        <tr><td style="background:#2C795A;border-radius:12px 12px 0 0;padding:32px 40px;text-align:center;">
        ...
        <tr><td style="background:#ffffff;padding:36px 40px;">
          ...
          <div style="background:#f9fafb;border:2px dashed #d1fae5;border-radius:8px;padding:20px;text-align:center;margin-bottom:28px;">
            <code style="font-size:24px;font-weight:700;letter-spacing:.2em;color:#2C795A;">${licenseKey}</code>
          </div>
          ...
          <table cellpadding="0" cellspacing="0" width="100%" style="margin-bottom:28px;">
            <tr>
              <td style="width:28px;vertical-align:top;padding-top:2px;">
                <div style="width:22px;height:22px;background:#2C795A;border-radius:50%;text-align:center;line-height:22px;font-size:12px;font-weight:700;color:#fff;">1</div>
              </td>
            ...
            <tr>
              <td style="width:28px;vertical-align:top;padding-top:2px;">
                <div style="width:22px;height:22px;background:#2C795A;border-radius:50%;text-align:center;line-height:22px;font-size:12px;font-weight:700;color:#fff;">2</div>
              </td>
              <td style="padding-left:10px;font-size:14px;color:#374151;line-height:1.5;">
            ...
            <tr>
              <td style="width:28px;vertical-align:top;padding-top:2px;">
                <div style="width:22px;height:22px;background:#2C795A;border-radius:50%;text-align:center;line-height:22px;font-size:12px;font-weight:700;color:#fff;">3</div>
              </td>
              <td style="padding-left:10px;font-size:14px;color:#374151;line-height:1.5;">
          ...
          <div style="text-align:center;margin-bottom:28px;">
            <a href="${redeemUrl}" style="display:inline-block;background:#2C795A;color:#ffffff;text-decoration:none;padding:14px 36px;border-radius:8px;font-weight:700;font-size:16px;">
          ...
          <div style="background:#f9fafb;border-radius:6px;padding:14px 16px;margin-bottom:8px;">
            ...
            <a href="${redeemUrl}" style="font-size:13px;color:#2C795A;word-break:break-all;">${redeemUrl}</a>
          </div>
        ...
        <tr><td style="background:#f9fafb;border-radius:0 0 12px 12px;padding:20px 40px;text-align:center;border-top:1px solid #e5e7eb;">
          <p style="margin:0 0 4px;font-size:12px;color:#9ca3af;">
            Questions? Reply to this email or visit <a href="${APP_URL}" style="color:#2C795A;">${APP_URL}</a>
          </p>
...
export async function sendAdminOrderNotification({
  ...
  const licenseRows = licenses
    .map(
      (l) =>
        `<tr><td style="padding:8px 12px;border-bottom:1px solid #e5e7eb;">${l.courseTitle}</td><td style="padding:8px 12px;border-bottom:1px solid #e5e7eb;font-family:monospace;color:#2C795A;">${l.key}</td></tr>`,
    )
    .join("");
  ...
  const { data, error } = await resend.emails.send({
    ...
    html: `
      <div style="font-family:Inter,sans-serif;max-width:600px;margin:0 auto;padding:32px 24px;background:#fff;border-radius:12px;border:1px solid #e5e7eb;">
        ...
        <a href="${APP_URL}/licenses" style="display:inline-block;background:#2C795A;color:#fff;text-decoration:none;padding:10px 22px;border-radius:8px;font-weight:600;font-size:14px;">View in Dashboard &rarr;</a>
      </div>
    `,
  });
  ...
  const { data, error } = await resend.emails.send({
    ...
    html: `
      <div style="font-family:Inter,sans-serif;max-width:580px;margin:0 auto;padding:32px 24px;background:#fff;border-radius:12px;border:1px solid #e5e7eb;">
        ...
        <a href="${resetUrl}" style="display:inline-block;background:#2C795A;color:#fff;text-decoration:none;padding:12px 28px;border-radius:8px;font-weight:600;font-size:15px;margin-bottom:24px;">Reset Password &rarr;</a>
        <p style="color:#9ca3af;font-size:13px;">Or paste this link in your browser:<br/><a href="${resetUrl}" style="color:#2C795A;">${resetUrl}</a></p>
